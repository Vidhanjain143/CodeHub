...
const ProblemDescription:React.FC<ProblemDescriptionProps> = ({problem,_solved}) => {
    ...
    const handleLike= async() =>{
      if(!user)
      {
        toast.error("You must logged in to like a problem",{position:"top-center",theme:"dark"})
      }
      await runTransaction(firestore,async(transaction)=>{
        const { problemDoc, userDoc, problemRef, userRef } = await returnUserDataAndProblemData(transaction);
        if(userDoc.exists() && problemDoc.exists()){
        if(liked){
            // remove problem id from likedproblems on userdocument,document likes on problem document
                ...
                className={` ${problemDifficultyClass} inline-block rounded-[21px] bg-opacity-[.15] px-2.5 py-1 text-xs font-medium capitalize `}>
                    ...
                    <div className='mt-4'>
                        {problem.examples.map((example,index)=>(
                            ...
                            {example.img &&(
                                <img className='mt-3' src={example.img} alt={problem.title} />
                            )}
                    ...
                    <div className='my-8 pb-4'>
                        ...
                        <ul className='text-white ml-5 list-disc my-2'>
